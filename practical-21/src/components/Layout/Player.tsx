...
const Player: React.FC = () => {
  ...
  const safePlay = (audio: HTMLAudioElement) => {
    ...
    // Avoid "Uncaught (in promise) AbortError: The play() request was interrupted by a call to pause()."
    promise?.catch((e: any) => {
  ...
  const totalDuration = currentTrack.duration
    ...
    : audioRef.current && Number.isFinite(audioRef.current.duration) && audioRef.current.duration > 0
      ...
      : '0:00';
  const safeArtistName = currentTrack.artistName ?? 'Unknown Artist';
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            >
              ...
              <div className="flex flex-col items-start min-w-0">
                ...
                <p className="text-xs md:text-sm text-gray-400 truncate max-w-[12rem] md:max-w-[16rem]">{safeArtistName}</p>
              </div>
            </button>
            ...
            <button
              ...
              onClick={() => {
                ...
                void toggleFavouriteByTrackId(currentTrack.id);
              }}
