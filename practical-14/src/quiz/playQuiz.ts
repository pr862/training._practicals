...
const quizList = document.getElementById('quizList') as HTMLDivElement;
const noQuizzesMessage = document.getElementById('noQuizzesMessage') as HTMLDivElement;
const loadingMessage = document.getElementById('loadingMessage') as HTMLDivElement;

const quizModal = document.getElementById('quizModal') as HTMLDivElement;
const closeModalBtn = document.getElementById('closeModalBtn') as HTMLButtonElement;

const modalQuizTitle = document.getElementById('modalQuizTitle') as HTMLHeadingElement;
const modalQuizDescription = document.getElementById('modalQuizDescription') as HTMLParagraphElement;

const quizQuestions = document.getElementById('quizQuestions') as HTMLDivElement;
const quizResults = document.getElementById('quizResults') as HTMLDivElement;
const scoreText = document.getElementById('scoreText') as HTMLParagraphElement;

const progressContainer = document.getElementById('progressContainer') as HTMLDivElement;
const answeredCountEl = document.getElementById('answeredCount') as HTMLSpanElement;
const currentScoreEl = document.getElementById('currentScoreDisplay') as HTMLSpanElement;
...
async function openQuiz(quiz: any) {
  ...
  try {
    ...
    questionsArray = fullQuiz.questions.map((q: any) => {
  ...
  questionsArray.forEach((q, index) => {
    ...
    div.querySelectorAll('input').forEach(input => {
      input.addEventListener('change', e => {
        answerQuestion(index, (e.target as HTMLInputElement).value);
      });
    });
  });
}

function answerQuestion(index: number, answerKey: string) {
...
function showFeedback(index: number) {
  ...
  options.querySelectorAll('label').forEach(label => {
    const key = (label.querySelector('input') as HTMLInputElement).value;
...
function updateProgress() {
  ...
  questionsArray.forEach((q, i) => {
    if (userAnswers[i] === q.correctKey) score++;
  });
...
function getSuggestion(percentage: number, category: string): { title: string; message: string; className: string } {
...
function displaySuggestion(percentage: number, category: string): void {
