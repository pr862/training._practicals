...
async function saveQuiz(): Promise<void> {
  ...
  for (let i = 0; i < questionItems.length; i++) {
    ...
    const questionText = (item.querySelector('.question-text') as HTMLInputElement).value.trim();
    const optionA = (item.querySelector('.option-a') as HTMLInputElement).value.trim();
    const optionB = (item.querySelector('.option-b') as HTMLInputElement).value.trim();
    const optionC = (item.querySelector('.option-c') as HTMLInputElement).value.trim();
    const optionD = (item.querySelector('.option-d') as HTMLInputElement).value.trim();
